...
// カードを捨て札に追加
const discardCardsToDeck = (cards) => {
...
io.on('connection', (socket) => {
    ...
    socket.on('draw-cards', () => {
        ...
        const cardsWithVisibility = hand.map((card, idx) => ({
            ...card,
    ...
    socket.on('showdown', () => {
        ...
        const activePlayers = gameState.players.filter(p => p.status === 'active' && p.hand && p.hand.length === 5);
